...
suite("ACP Real CLI Tool Call Smoke", () => {
  test("runs scripts/tool-call-e2e-test.mjs with real iflow cli", async function () {
    ...
    const result = await runNodeScript(rootDir, scriptPath, {
      ...process.env,
      IFLOW_ACP_PORT: process.env.IFLOW_ACP_PORT || "8125",
    ...
    assert.strictEqual(
      result.code,
      0,
      `tool call E2E test failed (code=${String(result.code)})\n${output}`,
    );
    assert.match(output, /\[tool-e2e\] summary/);
