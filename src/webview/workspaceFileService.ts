...
export class WorkspaceFileService {
  ...
  async listWorkspaceFiles(query: string): Promise<WorkspaceFileSummary[]> {
    ...
    const files = await this.deps.findFiles(pattern, excludePattern, WORKSPACE_FILE_SEARCH_LIMIT);
    this.deps.debug(`Workspace file search: query="${safeQuery}", results=${files.length}`);
  ...
  async readFiles(paths: string[]): Promise<AttachedFile[]> {
    ...
    this.deps.debug(`Reading attached files: count=${paths.length}, maxBytes=${maxBytes}`);
    ...
    for (const filePath of paths) {
      try {
        ...
        this.deps.debug(`Failed to read attached file: ${filePath}: ${message}`);
  ...
  async getWorkspaceFileList(cwd?: string, limit = WORKSPACE_FILE_LIST_DEFAULT_LIMIT): Promise<string[]> {
    const workspaceFolders = this.deps.getWorkspaceFolders();
    if (!workspaceFolders) {
      return [];
    }
    ...
    const normalizedLimit = Number.isFinite(limit) && limit > 0
      ? Math.floor(limit)
      : WORKSPACE_FILE_LIST_DEFAULT_LIMIT;
    const files = await this.deps.findFiles('**/*', excludePattern, normalizedLimit);
  ...
  resolveWorkspaceFolder(conversation: Conversation, activeEditor: vscode.TextEditor | undefined): string | undefined {
    ...
    if (conversation.workspaceFolderUri) {
      const stillExists = allFolders.some((folder) => folder.uri.fsPath === conversation.workspaceFolderUri);
      if (stillExists) {
  ...
  getAllWorkspaceFolderPaths(): string[] {
    return (this.deps.getWorkspaceFolders() ?? []).map((folder) => folder.uri.fsPath);
  }
