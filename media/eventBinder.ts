...
function closeConversationPanel(host: AppHost): void {
  host.appState.showConversationPanel = false;
  host.appState.conversationSearch = "";
  closeFloatingPanel("conversation-panel", "conversation-trigger");
}

function closeModeMenu(host: AppHost): void {
  host.appState.showModeMenu = false;
...
function attachConversationSearchListeners(host: AppHost): void {
  ...
  searchInput?.addEventListener("input", () => {
    host.appState.conversationSearch = searchInput.value;
    if (debounceTimer) {
    ...
    debounceTimer = setTimeout(() => {
      ...
      panel.outerHTML = renderConversationPanel({
        conversations: host.appState.getConversations(),
        search: host.appState.conversationSearch,
        showPanel: true,
        currentConversationId: host.appState.getCurrentConversationId(),
      });
      ...
      host.appState.showConversationPanel = true;
      ...
      if (newSearch) {
        newSearch.focus();
        newSearch.selectionStart = newSearch.selectionEnd =
...
export function attachFileChangeReviewListeners(host: AppHost): void {
  ...
  document.querySelectorAll("[data-file-change-action]").forEach((node) => {
    ...
    button.addEventListener("click", (event) => {
      ...
      const action = button.dataset.fileChangeAction as
        | "approve"
        | "rollback"
        | undefined;
...
export function attachTopBarListeners(host: AppHost): void {
  document
    .getElementById("conversation-trigger")
    ?.addEventListener("click", (event) => {
      event.stopPropagation();
      host.appState.showConversationPanel = !host.appState.showConversationPanel;
      (event.currentTarget as HTMLElement | null)?.setAttribute(
        "aria-expanded",
        host.appState.showConversationPanel ? "true" : "false",
      );
      const panel = document.getElementById("conversation-panel");
      if (!panel) {
        return;
      }
      panel.classList.toggle("hidden", !host.appState.showConversationPanel);
      if (host.appState.showConversationPanel) {
        const searchInput = document.getElementById(
          "conversation-search",
        ) as HTMLInputElement | null;
        searchInput?.focus();
  ...
  document
    .getElementById("new-conversation-top-btn")
...
export function attachModeListeners(host: AppHost): void {
  document
    .getElementById("mode-trigger")
    ?.addEventListener("click", (event) => {
      event.stopPropagation();
      host.appState.showModeMenu = !host.appState.showModeMenu;
      (event.currentTarget as HTMLElement | null)?.setAttribute(
        "aria-expanded",
        host.appState.showModeMenu ? "true" : "false",
      );
      const popup = document.getElementById("mode-popup");
      popup?.classList.toggle("hidden", !host.appState.showModeMenu);
    });
  ...
  if (thinkOption) {
    bindActivation(thinkOption, () => {
      const conversation = host.appState.getCurrentConversation();
      host.postMessage({
        type: "setThink",
        enabled: !(conversation?.think ?? false),
      });
    });
  }
  ...
  if (modelSelect) {
    ...
    modelSelect.addEventListener("change", () => {
      host.postMessage({
        type: "setModel",
        model: modelSelect.value as ModelType,
      });
      host.autoSizeSelect(modelSelect);
    });
  }
...
export function attachComposerListeners(host: AppHost): void {
  ...
  if (host.appState.pendingConfirmation) {
    attachApprovalListeners(host);
    return;
  }
  if (host.appState.pendingQuestion) {
    attachQuestionListeners(host);
    return;
  }
  if (host.appState.pendingPlanApproval) {
...
export function attachFileOpenListeners(
  postMessage: (msg: WebviewMessage) => void,
): void {
...
export function attachIDEContextListeners(host: AppHost): void {
  document.querySelectorAll(".ide-context-dismiss").forEach((button) => {
    button.addEventListener("click", () => {
      const type = (button as HTMLElement).dataset.dismiss as
        | "activeFile"
        | "selection"
        | undefined;
      if (!type) {
      ...
      host.appState.ideContextDismissed = {
        ...host.appState.ideContextDismissed,
        [type]: true,
      };
...
export function closePanelsOnOutsideClick(
  host: AppHost,
