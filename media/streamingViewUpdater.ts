...
export function updateIDEContextChipsView(options: {
  ...
  const newHtml = renderIDEContextChips(options.ideContext, options.ideContextDismissed);
  if (existing) {
...
export function updateComposerStatusBarView(options: {
  ...
  if (modeTrigger) {
    ...
    modeTrigger.setAttribute('aria-expanded', options.showModeMenu ? 'true' : 'false');
  }
...
export function updatePendingIndicatorView(options: {
  ...
  const target = options.container ?? document.getElementById('messages-container');
  if (!target) {
    return;
  }
  ...
  if (options.isStreaming) {
    const indicatorHtml = renderPendingIndicator(options.faviconUri, options.pendingStatusText);
