...
import { isApprovalOutcome } from "./panelTypes";
...
export function attachApprovalListeners(host: AppHost): void {
  const conf = host.appState.pendingConfirmation;
  if (!conf) return;
  ...
  const handleApproval = (outcome: "allow" | "alwaysAllow" | "reject") => {
    ...
    host.appState.pendingConfirmation = null;
    host.render();
  };

  document
    .querySelectorAll(".approval-option[data-approval]")
    .forEach((btn) => {
  ...
  const keyHandler = (e: KeyboardEvent) => {
    ...
    if (e.key === "1") {
      e.preventDefault();
      handleApproval("allow");
    } else if (e.key === "2") {
      e.preventDefault();
      handleApproval("alwaysAllow");
    } else if (e.key === "3") {
      e.preventDefault();
      handleApproval("reject");
    } else if (e.key === "Escape") {
