...
function truncateLines(lines: DiffLine[]): DiffLine[] {
...
export function extractEditedFileDiffFromPatch(
  block: ToolBlock,
): EditedFileDiff | null {
  ...
  return {
    ...
    added,
    removed,
    lines: truncateLines(renderedLines),
  };
}

export function extractEditedFileDiffFromOldNew(
  block: ToolBlock,
): EditedFileDiff | null {
  const input = block.input || {};
  const oldStr = getInputString(input, [
    "old_string",
    "oldString",
    "old_str",
    "old_text",
  ...
  const newStr = getInputString(input, [
    "new_string",
    "newString",
    "new_str",
    "new_text",
  ...
  return { fileName, added, removed, lines: truncateLines(renderedLines) };
}

export function extractEditedFileDiff(block: ToolBlock): EditedFileDiff | null {
  return (
    extractEditedFileDiffFromPatch(block) ||
    extractEditedFileDiffFromOldNew(block)
  );
}
